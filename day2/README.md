# Persistent State, Jobs, Configuration, Secrets, and Service Accounts
...
When you're ready to run your new Jobs use `kubectl apply` to create all of the resources, then use `kubectl get jobs`, `kubectl get pods`, and `kubectl logs -l app=ping` and `kubectl logs -l app=traceroute` to see the results. When the Jobs are done they will move to a completed state.
...
You can use `kbuectl run` to sneak a peek at the JWT mounted in as a secret. That secret is located at `/var/run/secrets/kubernetes.io/serviceaccount/token`.
...
Doing so will expose a secret similar to this:

```
eyJhbGciOiJSUzI1NiIsImtpZCI6IiJ9.eyJpc3MiOiJrdWJlcm5ldGVzL3NlcnZpY2VhY2NvdW50Ii
.fazacOVl6XC
P_b8KwxDzsltrfeBmOK_yO22QMG-B3zoZs6zkTf8wv1czqfQzBmBuwZR0G5oyge1dJaefT0hS9Y6dYs
NlutWnenYC4FYGTpTlYUp9hr1gYJ6v92jWGRRuuiL9Wac7ZSurk0r9oH9d5pdaJPOEGjyEATHoxuZZ6
fHUeyVG8s6ikajKNxl6wYg7bsTeUqU1JMx9NA5W56qwWovmBrlhhwNcvoxVpBU8_UWnr8ypA9XUEXYd
tg_fZPNJyKo0v1ZCGjhqon_ObUvV4BRBml_81hwi_PvT4zz0HzuilOBfgHTZy7QqaDrJd2nA6RhIZW5
c6y-73lF1XyyD8w
```

The API server serves requests over HTTPS where the server certificate is issued by a private certificate authority. Client's need access to the CA certificate in order to be able to authenticate the server. That CA certificate is provided to Pods at a well known location: `/var/run/secrets/kubernetes.io/serviceaccount/ca.crt`. You can read the certificate generated by your local Kubernetes cluster by running the following:
...
kubectl run peek \
  ...
  --restart=Never \
  --image alpine:3.8 \
  -- sh -c "cat /var/run/secrets/kubernetes.io/serviceaccount/ca.crt"
```

```
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
```

Finally, with credentials the last thing a client would need to do to call the Kubernetes API from within a Pod is be able to resolve an endpoint on the network. The Kubernetes API runs as a Service in the `default` namespace. You can work with that resource just like any other: `kubectl get svc --namespace default`. But from within a Pod things are a bit trickier. By default Kubernetes publishes basic discovery information as environment variables, but using those is sort poor form since environment variables are static. 

All Services are resolvable via DNS and the Kubernetes service is no exception. The name is "kubernetes" and it is in the "default" namespace. So, by convention the service will be resolvable globally using the fully-qualified name: `kubernetes.default.svc`. Pods in the "default" namespace would be able to resolve the service by "kubernetes" because all Pods in the same namespace have their namespace suffix included in their DNS search domain.
...
kubectl run peek \
  ...
  --restart=Never \
  --image alpine:3.8 \
  -- sh -c "nslookup kubernetes.default.svc"
```

... results in the following output (or something similar):
...
With credentials and an endpoint a program running in a Pod would have everything it needs to implement controller behavior, automate operator processes, or provide third-party system integration (like propagating events to Slack, etc).

### Exercise 7: Create a ServiceAccount and Role and Prevent Launching a Pod from within a Pod

In this challenge use the `default` service account to create a Pod from within a naked Pod that you created.

You can get the kubectl binary for Linux from [dl.k8s.io - the official Kubernetes site](https://dl.k8s.io/v1.10.6/bin/linux/amd64/kubectl) or by running `curl -L https://dl.k8s.io/v1.10.6/bin/linux/amd64/kubectl -o /usr/local/bin/kubectl` in your Dockerfile or running container.

Next, create a ServiceAccount, Role, and RoleBinding such that you will not be able to create a Pod from within a Pod if it uses the ServiceAccount.
